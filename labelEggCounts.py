...
class EggCountLabeler:
        ...
        with open(self.countsFile) as myFile:
            ...
            for i, row in enumerate(reader):
                if len(row) == 1:
                    ...
                    getCtsFromFile = (
                        ctUnknown or cts[currentImg] != CT.fourCircle.name)
    ...
    def getIndicesOfIgnoredImages(self):
        for idx in self.imgs_to_ignore: 
            ...
            self.abs_idxs_ignored = self.abs_idxs_ignored.union(set(
                range(abs_start_index, abs_start_index + len(self.subImgs[idx]))))
    ...
    def loadSubImages(self):
        ...
        if filesExist[3]:
            with open(self.ignoreFilePath, 'r') as my_f:
                self.imgs_to_ignore = [self.lowerBasenames.index(img_f.lower())
    ...
    def getUnfilledRegions(self):
        ...
        if [] in self.eggCounts and len(trueRegionCts) == 0:
            ...
            for i in range(len(self.eggCounts)):
                if i not in self.imgs_to_ignore and i > 0:
            ...
            while leading_imgs_not_processed:
                leading_imgs_not_processed = any(
                    [self.imgPaths[i] not in self.processingCompleted for i in range(first_non_ignored_img + 1)])
                time.sleep(0.3)
            ...
            for tReg in list(trueRegionCts):
                if tReg.start <= idx_end and tReg.start >= idx_start:
                    if tReg.stop - 1 > idx_end:
                        trueRegionCts[trueRegionCts.index(
                            tReg)] = slice(idx_end, tReg.stop)
                    else:
        ...
        if len(trueRegionCts) == 0:
            for i in range(len(self.eggCounts)):
                if i not in self.imgs_to_ignore:
                    ...
                    self.eggCountsConcat = np.asarray(concat(self.eggCounts))
                    if len(trueRs) > 0:
                        return [slice(self.absolutePosition(i, trueRs[0].start),
    ...
    def findMismatchBetweenSavedCountsAndClickedEggs(self):
        ...
        unofficialEggCounts = [i for j, i in enumerate(
            unofficialEggCounts) if j not in self.abs_idxs_ignored and j < cumulativeFrontierIdx]
        eggCountsConcatTrimmed = [i for j, i in enumerate(
            self.eggCountsConcat) if j not in self.abs_idxs_ignored and j < cumulativeFrontierIdx]
        abs_idxs_trimmed = [i for i in abs_idxs if i not in self.abs_idxs_ignored and i < cumulativeFrontierIdx]
        foundMismatches = not np.array_equal(
            unofficialEggCounts, eggCountsConcatTrimmed)
        if foundMismatches:
            self.preImgIdx, self.preSubImgIdx = self.getImgAndSubImgIdx(abs_idxs_trimmed[np.argmax(
                unofficialEggCounts != eggCountsConcatTrimmed)])
        ...
        if k == 13:
            ...
            if self.clickLabelManager.getNumClicks(self.lowerBasenames[self.imgIdx],
                ...
                self.clickLabelManager.addKey(
                    self.lowerBasenames[self.imgIdx], self.rowNum(
                    ), self.colNum(),
                    self.wellPosition)
    ...
    def showIgnoreLabel(self):
        ...
        textDims = cv2.getTextSize('Image can be skipped', cv2.FONT_HERSHEY_PLAIN, 1.8, 2)[0][
            ::-1]
        putText(overlay, 'Image can be skipped', (0.5*(self.eggsImg.shape[1] - textDims[1]), 0.5*(
            self.eggsImg.shape[0] - textDims[0])), (0, 1), textStyle(1.8, COL_R_D, 2))
        alpha = 0.4
    ...
    def absolutePosition(self, idx=None, subImgIdx=None):
        """Return index of the current sub-image."""
        if idx == None:
            idx = self.imgIdx
        if subImgIdx == None:
            subImgIdx = self.subImgIdx
        return sum([len(eggCountList) for eggCountList in self.subImgs[
            :idx]]) + subImgIdx
