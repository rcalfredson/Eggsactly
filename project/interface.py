...
def send_csv(filename):
    ...
    print("file to download:", fileToDownload)
    print("is it a file?", os.path.exists(fileToDownload))
    print(
        "is the version without the relative step-up a file?",
        os.path.exists(os.path.join("temp", filename)),
    )
...
@socketIO.on("remove-img")
def remove_img(data):
    del sessions[data["sid"]].predictions[
