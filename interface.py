...
def handle_upload():
    ...
    for i, file in enumerate(request.files):
        ...
        while True:
            try:
                process_img(i, file, sid, n_files, attempts)
                succeeded = True
            ...
            if succeeded:
                ...
                sessions[sid].report_counting_error(sessions[sid].imgPath,
                                                    CUDAMemoryException)
    pauser.set_resume_timer()
...
def process_img(i, file, sid, n_files, attempts):
    if file and allowed_file(file):
        ...
        socketIO.emit('counting-progress',
                      ...
                      room=sid)
        sessions[sid].segment_img_and_count_eggs(filePath)
