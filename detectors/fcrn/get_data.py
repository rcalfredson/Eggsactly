...
def create_hdf5(dataset_name: str,
    ...
    if multiSize:
        if train_size > 0:
            trainDataFile = h5py.File(os.path.join(dataset_name, 'train.h5'), 'w')
        else:
            trainDataFile = None
...
def get_egg_image_paths(held_out=False):
    all_images = glob('egg_source%s/*_dots*'%('/heldout_robert_WT_1' if held_out else ''))
    shuffle(all_images)
    return all_images
...
def generate_egg_heldout_data():
    ...
    in_channels = 3
    _, valid_h5 = create_hdf5('egg-eval-robert-wt_1',
                              train_size=0,
                              valid_size=None,
                              img_size=(None, None),
...
def generate_egg_datasets():
    ...
    # generate_egg_data(mode='fullSize')
    generate_egg_data(mode='patches')

def generate_egg_data(mode='patches'):
    """Generate HDF5 files for egg-laying images."""
    multiSize = mode in ('fullSize', 'combined',)
    datasetSuffix = {'patches': '', 'fullSize': '-fullsize',
                     'combined': '-combined'}[mode]
    # image names need to be sourced from both fullsize and patch directories
    fullSizeDirs = ['egg_source/fullsize_2020-12-15/%s/*_dots*'%setType for setType in (
        'train', 'valid')]
    if mode == 'patches':
        training_image_names = glob('egg_source/train/*_dots*')
        # temporary downsizing of the dataset
        # training_image_names = list(np.random.choice(training_image_names, 1500, replace=False))
        validation_image_names = glob('egg_source/valid/*_dots*')
        ...
        training_image_names = list(np.random.choice(training_image_names,
            int(0.25*len(training_image_names)), replace=False))
        validation_image_names = list(np.random.choice(validation_image_names,
            int(0.25*len(validation_image_names)), replace=False))
    train_h5, valid_h5 = create_hdf5('eggTemp%s'%datasetSuffix,
    ...
    def fill_h5(h5, images, oneDSetPerImg=False):
        ...
        for i, label_path in enumerate(images):
            ...
            if oneDSetPerImg:
                basename = os.path.basename(label_path).replace('_dots.png', '')
                h5.create_dataset('%s_dots' % basename, (1, 1,
                    *label.shape[0:2]))
