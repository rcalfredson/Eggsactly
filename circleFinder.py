import itertools, operator, os, math, threading
import cv2
import timeit
...
def setup_cfg():
  ...
  cfg.merge_from_list(['MODEL.WEIGHTS', os.path.join(dirname,
    'models/arena_pit.pth')])
...
def fake_image_corners(xy_sequence):
    ...
    all_x, all_y = [tuple(xy_sequence[:, :, i].flatten()) for i in range(
      xy_sequence.shape[-1])]
...
def getChamberTypeByRowsAndCols(numRowsCols):
  ...
  """
  for ct in CT:
    if numRowsCols[0] == ct.value().numRows and numRowsCols[1] == \
        ct.value().numCols:
      return ct.name
...
class CircleFinder:
  ...
  def getPixelToMMRatio(self):
    ...
    self.pxToMM = 0.5*(self.avgDists[1]/CT[self.ct].value().rowDist +\
  ...
  def processDetections(self):
    ...
    for detI, detections in enumerate((self.xDetections, self.yDetections)):
      ...
      for i, trueRegion in enumerate(binClusters):
        self.wellCoords[detI].append(int(round(np.mean([detections[(
    ...
    for i in range(len(self.wellCoords)):
      ...
      self.wellCoords[i] = reject_outliers_by_delta(np.asarray(
        self.wellCoords[i]))
    ...
    self.sortedCentroids = np.array(self.sortedCentroids).reshape((tuple(
      reversed(self.numRowsCols)) + (() if None in self.sortedCentroids else \
      (-1,))))
    ...
    if height_skew / self.imageResized.shape[0] > 0.01 or \
        width_skew / self.imageResized.shape[1] > 0.01:
      self.skewed = True
    else:
      self.skewed = False
    if self.skewed and not self.allowSkew:
      print('Warning: skew detected in image %s. To analyze '%self.imgName + \
    ...
    for i, col in enumerate(self.sortedCentroids):
      for j, centroid in enumerate(col):
        ...
        if len(centroid[np.isnan(centroid)]):
          row = self.sortedCentroids[:, j]
          self.sortedCentroids[i, j] = linearIntersection(dict(
            row=self.rowRegressions[j],
  ...
  def findCircles(self):
    ...
    """
    self.imageResized = cv2.resize(self.img, (0, 0), fx=0.15, fy=0.15,
      interpolation=cv2.INTER_CUBIC)
    predictions, _ = model.run_on_image(self.imageResized)
    predictions = [predictions['instances'].pred_masks.cpu().numpy()[i, :, :
    ...
    if self.ct is not CT.fourCircle.name:
      ...
      for i, centroid in enumerate(self.centroids):
        self.centroids[i] = rotate_around_point_highperf(centroid,
          rotationAngle, image_origin)
    self.processDetections()
    wells = np.array([np.round(np.divide(well, 0.15)).astype(int) for well in \
      self.sortedCentroids])
    for i in range(len(self.wellCoords)):
      self.wellCoords[i] = np.round(np.divide(self.wellCoords[i], 0.15)
        ).astype(int)
    wells = wells.reshape(self.numRowsCols[0]*self.numRowsCols[1], 2
      ).astype(int)
    self.wells = wells
    ...
    return (wells, self.avgDists, self.numRowsCols, self.rotatedImg,
      self.rotationAngle)
...
def calculateRegressions(row, col):
  ...
  colInd = np.array([el[1] for el in col if not np.isnan(el).any()]
    ).reshape(-1, 1)
  colModel.fit(colInd, [el[0] for el in col if not np.isnan(el).any()])
  rowModel = LinearRegression()
  rowInd = np.array([el[0] for el in row if not np.isnan(el).any()]
    ).reshape(-1, 1)
  ...
  return dict(row=dict(slope=b, intercept=d),
    col=dict(slope=a, intercept=c))
...
def linearIntersection(regressions):
  ...
  return ((interceptDiff)/(slopeDiff), r['col']['slope']*interceptDiff\
    /slopeDiff + r['col']['intercept'])
