...
import numpy as np, math, scipy.stats as st, scipy.signal as signal
import pickle
...
def parseIntList(l):
  ...
  for p in ps:
    ...
    ints.extend((fl[0],) if len(fl) == 1 else list(range(fl[0], fl[1]+1)))
  return ints
...
# note: numpy seems not to have a function that stops on the first true
def firstTrue(a):
  i = np.argmax(a)
  return i if a[i] else None
...
# returns rolling averages of given array and window size.
def movingAverage(a, windowSize):
  window = np.full(int(windowSize), 1./windowSize)
...
# returns array reduced to finite elements
def toFinite(a): return a[np.isfinite(a)]

# returns maximum for k=0, next element for k=1, etc.
# note: fast (O(n))
def maxk(a, k, axis=None):
  a = np.array(a)
  k1 = a.size - 1 - k
  return np.partition(a, k1, axis=axis)[k1]

def _maxkTest():
  ts = [(([1, 4, 2], 0), 4), (([0, 10], 1), 0)]
  for args, r in ts:
    test(maxk, args, r)
...
# formats a fullfile command as a string to be passed to MATLAB
# using inputs of path and filename.
def fullfile(path, filename):
  return "fullfile('" + path + "', '" + filename + "')"

# returns the difference between two angles within a 180-degree range.
# e.g. angleDist(270, 0) => 90 and angleDist(0, -380) => 20.
# option abs=True returns absolute value of result.
def angleDiff(theta, theta2=None, absVal=False):
  result = normAngles(np.diff(theta), False) if theta2 == None else \
    normAngles(theta - theta2, False)
  return np.abs(result) if absVal == True else result

# normalizes the given angles to be between -pi and pi
def normAngles(a, useRadians=True):
  return np.mod(a+np.pi, 2*np.pi) - np.pi if useRadians else \
    np.mod(a + 180, 360) - 180
...
# returns Butterworth low-pass filtered data using the given order (N)
# and normalized frequency cutoff (Wn)
def butter(x, N=1, Wn=0.5):
  xSmoothed = np.copy(x)
  b, a = signal.butter(N, Wn)
  finiteRegions = trueRegions(~np.isnan(x))
  for region in finiteRegions:
    xSmoothed[region] = signal.lfilter(b, a, x[region])
  return xSmoothed
...
# renames the given file; a possibly existing file with the new name is deleted
def rename(fn, newfn):
...
def readCsv(fn, toInt=True, nCols=None):
  ...
  with open(fn) as f:
    ...
    return [list(map(int, row)) for row in dt] if toInt else dt
...
def combineImgs(imgs, nc=10, d=10, hdrs=None, style=None, hd=3, adjustHS=True,
  ...
  for i, img in enumerate(imgs):
    r, c = divmod(i, nc)
    xL, yL = int(d/2 + c*(wA+d)), int(d/2+hdrLH+hdrLD+hdrH+hd + r*(h+d+hdrH+hd))
    if img is not None:
      h1, w1 = img.shape[:2]
...
# returns indices into the given array for the start of any matches found
# with the given search sequence.
def seqMatches(arr, seq):
  S = cv2.matchTemplate(arr.astype('uint8'), seq.astype('uint8'), cv2.TM_SQDIFF)
  thresh = 1e-5
  idx = np.where(S.ravel() < thresh)[0]
  if len(idx)>0:
    return np.unique((idx[:,None] + np.arange(seq.size)).ravel()).astype(int)
  else:
    return np.asarray([])
...
def textStyle(size=.9, color=COL_BK, thickness=1):
  return (cv2.FONT_HERSHEY_PLAIN, size, color, thickness, CV_AA)
