...
def drawResults(img, results):
  global bxs_final
  d_bxs, d_scores = results['detection_boxes'][0].numpy(), results['detection_scores'][0].numpy()
...
for imgPath in imgs:
  ...
  results = detector.detectObjectsInImage(img)
  print('finished call to predictor')
