...
def options():
    p = argparse.ArgumentParser(
        description="Sample patches of images and "
        + "accompanying patch annotations to use for FCRN training"
    )
    p.add_argument(
        "--keep",
        help="Keep only images on the newline-"
...
sourceImgDirs = ["P:/Egg images_9_3_2020/WT_5 - Copy"]
picklePaths = [
...
for i, picklePath in enumerate(picklePaths):
    ...
    for clickKey in loadedData["clicks"]:
        ...
        clickCounter += 1
        if (
            img in loadedData["frontierData"]["fileName"]
            and loadedData["frontierData"]["subImgIdx"] <= clickCounts[img]
        ):
            continue
        ...
        print(
            ...
            loadedData["frontierData"]["fileName"],
...
def saveClickPositionImage(patch, outputPath):
    subImageKey = clickLabelManager.subImageKey(
        os.path.basename(patch.imgPath), patch.rowNum, patch.colNum, patch.position
    )
    ...
    clicksInRange = tuple(
        [
            tuple([int(coord) - (y1 if i else x1) for i, coord in enumerate(click)])
            for click in clicks
            if MODE == "full"
            or (
                click[0] > x1
                and click[0] < x1 + SIDE_LENGTH
                and click[1] > y1
                and click[1] < y1 + SIDE_LENGTH
            )
        ]
    )
...
class Patch:
    def __init__(self, imgPath, sourceImg, rowNum, colNum, position=None):
        ...
        if MODE == "patch":
            ...
            self.sampleArea = sourceImg[
                self.yOffset : self.yOffset + SIDE_LENGTH,
        ...
        self.basename = "%s_%i_%i_%s%s" % (
            os.path.basename(imgPath.split(".jpg")[0]),
            rowNum,
            colNum,
        ...
        if MODE == "full":
            if MODE_FOR_FULL == "heldout":
                destPath = os.path.join(
                    DEST_PATH, "%s_robert_WT_5_debug" % MODE_FOR_FULL
                )
            else:
                destPath = os.path.join(
...
for i, imgPath in enumerate(sourceImgs):
    ...
    if parentPath in imgMetadata and lbn in imgMetadata[parentPath]:
        ...
        rotatedImg = rotate_image(img, imgMetadata[parentPath][lbn]["rotationAngle"])
...
for imgPath in subImgs:
    for i, subImg in enumerate(subImgs[imgPath]):
        if (
            imgPath in loadedData["frontierData"]["fileName"]
            and i >= loadedData["frontierData"]["subImgIdx"]
        ):
            continue
        ...
        if class_rand_var < heldOutProb:
            ...
            class_rand_var >= heldOutProb
            and class_rand_var < heldOutProb + fullSizeProb
        ):
            MODE = "full"
