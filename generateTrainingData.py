...
DEST_PATH = '/media/Synology3/Egg images_9_3_2020/with_larvae/subimgs'
...
for group in ('train', 'valid'):
    ...
    def mkDestDir(dirName):
        Path(os.path.join(DEST_PATH, dirName)).mkdir(parents=True,
                                                     exist_ok=True)
...
sourceImgDirs = ['/media/Synology3/Egg images_9_3_2020/with_larvae']
picklePaths = [
    '%s/egg_count_labels_robert.pickle' % sourceImgDir
    for sourceImgDir in sourceImgDirs
]
...
for i, picklePath in enumerate(picklePaths):
    ...
    for clickKey in loadedData['clicks']:
        ...
        if img in clickCounts:
            ...
            clickCounts[img] = 0
        clickCounter += 1
        if not loadedData['frontierData'][
                'finishedLabeling'] and img in loadedData['frontierData'][
                    'fileName'] and loadedData['frontierData'][
                        'subImgIdx'] <= clickCounts[img]:
            continue
        ...
        print('frontier information:', loadedData['frontierData']['fileName'],
              loadedData['frontierData']['subImgIdx'])
...
if opts.keep:
    ...
    badImgs = [('%s.jpg' % imgName).lower()
               for imgName in ('Apr5_2left', 'Apr5_2right', 'Apr5_3left',
                               'Apr5_3right', 'Apr5_5left', 'Apr5_5right',
                               'Apr5_9left', 'Apr5_9right', 'Apr7_10left',
                               'Apr7_10right', 'Apr7_2left', 'Apr7_2right',
                               'Apr7_3left', 'Apr7_9right', 'Apr7_9left',
                               'Apr7_3right')]
...
def saveClickPositionImage(patch, outputPath):
    ...
    clicksInRange = tuple([
        tuple(
            [int(coord) - (y1 if i else x1) for i, coord in enumerate(click)])
        for click in clicks
        if MODE == 'full' or (click[0] > x1 and click[0] < x1 + SIDE_LENGTH and
                              click[1] > y1 and click[1] < y1 + SIDE_LENGTH)
    ])
...
class Patch:
    def __init__(self, imgPath, sourceImg, rowNum, colNum, position=None):
        ...
        if MODE == 'patch':
            ...
            self.sampleArea = sourceImg[self.yOffset:self.yOffset +
                                        SIDE_LENGTH,
        ...
        self.basename = '%s_%i_%i_%s%s' % (os.path.basename(
            imgPath.split('.jpg')[0]), rowNum, colNum, '' if position == None
                                           else '%s_' % position, randID())
        ...
        if MODE == 'full':
            if MODE_FOR_FULL == 'heldout':
                ...
                destPath = os.path.join(
                    DEST_PATH, '%s_fullsize_%s' % (MODE_FOR_FULL, GROUP))
        else:
...
for i, imgPath in enumerate(sourceImgs):
    ...
    if parentPath in imgMetadata and lbn in imgMetadata[parentPath]:
        ...
        rotatedImg = rotate_image(
            img, imgMetadata[parentPath][lbn]['rotationAngle'])
        subImgs[imgPath] = subImagesFromBBoxes(
            rotatedImg, imgMetadata[parentPath][lbn]['bboxes'])
    else:
        ...
        subImgs[imgPath], bboxes = circleFinder.getSubImages(
...
for imgPath in subImgs:
    for i, subImg in enumerate(subImgs[imgPath]):
        if imgPath in loadedData['frontierData'][
                'fileName'] and i >= loadedData['frontierData']['subImgIdx']:
            continue
